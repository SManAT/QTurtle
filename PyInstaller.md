...
When running the built `.exe`, you may see:

```
File "...\dist\QTurtle\_internal\ui\Ui_MainWindow.py", line 23, in <module>
    from editor import CodeEditor
ModuleNotFoundError: No module named 'editor'
```

**Cause:** `pyside6-uic` generates a bare `from editor import CodeEditor` import. This works when running from source (because `src/qturtle/` is on `sys.path`), but inside the PyInstaller bundle only fully-qualified package names are available.
...
**Note:** This file is auto-generated by `pyside6-uic`. To make the fix permanent, open `Ui_MainWindow.ui` in Qt Designer, go to **Edit → Beförderungsverwaltung** (Promote Widgets), select `CodeEditor`, and set the **Header-Datei** (Header file) to `qturtle.editor` instead of `editor`. After that, every regeneration will produce the correct import automatically.
